---
title: "The Benefits of and Motivations Behind Large-Team Coordination in Psychology - Supplementary Materials"
author:
  - name: Sajedeh Rasti
    corresponding: true
...
bigauthor_performance_5_15<- readRDS(here("data","author_smallvsbig_performance_citation_metrics_15.rds"))
bigauthor_performance_sjr5_15<- readRDS(here("data","author_smallvsbig_performance_journal_ranking_15.rds"))
bigauthor_performance_5_25<- readRDS(here("data","author_smallvsbig_performance_citation_metrics_25.rds"))
bigauthor_performance_sjr5_25<- readRDS(here("data","author_smallvsbig_performance_journal_ranking_25.rds"))

dunn_five<- readRDS(here("data","dunn_five.rds"))
dunn_ten<- readRDS(here("data","dunn_ten.rds"))
dunn_fif<- readRDS(here("data","dunn_fif.rds"))

dunn_five2<- readRDS(here("data","dunn_five2.rds"))
dunn_ten2<- readRDS(here("data","dunn_ten2.rds"))
dunn_fif2<- readRDS(here("data","dunn_fif2.rds"))
  ...
  Keywords = c(
    ...
    paste(
      c(
        ...
        "https://openalex.org/W4238875611",
...
Because papers could receive multiple codes, we calculated inter-rater reliability at the code level rather than requiring agreement on the complete set of codes assigned to each paper. Each code was treated as a separate binary decision (present or absent), with agreement recorded when both coders assigned the code or when both agreed that it was absent. We calculated overall agreement as the mean percentage agreement across all coding categories. @tbl-first-agreement summarizes agreement from the first round of coding, prior to adjustments to the codebook.

```{r}
#| label: tbl-first-agreement
#| cache: true
#| tbl-cap: Initial Agreement on the code level
#| disable-apaquarto-processing: true


intercoder_initial <- intercoder_initial |>
  rename(`Agreement (%)` = Percent_Agreement)

overall <- intercoder_initial |>
  summarise(
    Code = "Overall",
    N = mean(N),
...
intercoder_initial <- bind_rows(intercoder_initial, overall)
...
After refining the coding criteria, agreement improved across almost all codes (see @tbl-second-agreement). The main source of disagreement concerned distinguishing papers that were part of a larger consortium from those involving substantial financial and logistic interdependencies. To resolve this ambiguity, we refined the coding procedure such that the Methods section of the paper was consulted when the abstract did not clearly indicate whether the data had been collected specifically for the study or originated from an existing consortium or larger research project.


```{r}
#| label: tbl-second-agreement
#| cache: true
#| tbl-cap: Final Agreement on the code level
#| disable-apaquarto-processing: true

intercoder_final <- intercoder_final |>
  rename(`Agreement (%)` = Percent_Agreement)

overall <- intercoder_final |>
  summarise(
    Code = "Overall",
    N = mean(N),
    `Agreement (%)` = mean(`Agreement (%)`, na.rm = TRUE)
...
intercoder_final <- bind_rows(intercoder_final, overall)
...
ranking_ten <- full_join(group_scores_ten, group_losses_ten, by = c("winner" = "loser")) |>
  rename("Group size of 10" = winner) |>
  mutate(wins = replace_na(wins, 0),
         losses = replace_na(losses, 0),
         overall = wins - losses) |>
  arrange(desc(overall))
...
ranking_fif <- full_join(group_scores_fif, group_losses_fif, by = c("winner" = "loser")) |>
  rename("Group size of 15" = winner) |>
  mutate(wins = replace_na(wins, 0),
         losses = replace_na(losses, 0),
         overall = wins - losses) |>
  arrange(desc(overall))
...
flextable(ranking_five) |>separate_header()|> fontsize(size = 7.5)|>fontsize(size = 7.5, part = "header")|>align(align = "center", part = "all")|>autofit()|>bold(part = "header")
flextable(ranking_ten) |>separate_header()|>fontsize(size = 7.5)|>fontsize(size = 7.5, part = "header")|>align(align = "center", part = "all")|>autofit()|>bold(part = "header")
flextable(ranking_fif) |>separate_header()|>fontsize(size = 7.5)|>fontsize(size = 7.5, part = "header")|>align(align = "center", part = "all")|>autofit()|>bold(part = "header")
...
#| warning: false
#| cache: true
#| tbl-cap: Rank ordering groups of authors across different team size definition based on their performance in Dunn test of second-year citation
#| layout-ncol: 2
#| layout-nrow: 2
#| disable-apaquarto-processing: true
...
#| fig-cap: "Team size of five and total citation, second-year citation, and date-corrected citation counts"
#| fig-align: center
#| fig-width: 8
#| fig-height: 8
...
p2<-dat |>
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))+
  labs(x = " ", y= "Second-year citation count")+
  ggtitle("B")
...
#| fig-cap: "Team size of ten and total citation and date-corrected citation counts"
#| fig-align: center
#| fig-width: 8
#| fig-height: 6
...
#| fig-cap: "Team size of fifteen and total citation, second-year citation, and date-corrected citation counts"
#| fig-align: center
#| fig-width: 8
#| fig-height: 8
...
#| fig-cap: "Percentiles of total citation, second-year citation, date-corrected citation, and SJR metrics based on group size of 5"
#| fig-align: center
#| fig-width: 8
#| fig-height: 8
#| cache: true
...
p1_legend<-dat|>filter(n_author<20)|>group_by(author_group_five)|> summarise(value= quantile(citation, c(.7, .8, .90, .99)))|> mutate(percentile = as.factor(c(.7, .8,.9,.99)))|>
  ggplot(aes(x=author_group_five, y = value, color = percentile))+
  geom_point(size= 2)+
  theme_classic(base_size = 11)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))+
  labs(x= " ", y= "Total citation")+
...
p1<- dat|>filter(n_author<20)|>group_by(author_group_five)|> summarise(value= quantile(citation, c(.7, .8, .90, .99)))|> mutate(percentile = as.factor(c(.7, .8,.9,.99)))|>
  ggplot(aes(x=author_group_five, y = value, color = percentile))+
  geom_point(size= 2)+
  theme_classic(base_size = 11)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))+
...
p2<-dat|>filter(n_author<20)|>group_by(author_group_five)|> summarise(value= quantile(citation_2y, c(.7, .8, .90, .99)))|> mutate(percentile = as.factor(c(.7, .8,.9,.99)))|>
  ggplot(aes(x=author_group_five, y = value, color = percentile))+
  geom_point(size= 2)+
  theme_classic(base_size = 11)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))+
...
p3<- dat|>filter(n_author<20)|>group_by(author_group_five)|> summarise(value= quantile(corr_citation, c(.7, .8, .90, .99)))|> mutate(percentile = as.factor(c(.7, .8,.9,.99)))|>
  ggplot(aes(x=author_group_five, y = value, color = percentile))+
  geom_point(size= 2)+
  theme_classic(base_size = 11)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))+
  labs(x= " ", y= "Date-corrected citation")+
  scale_color_manual(values = c("#2f4b7c", "#d45087", "#f95d6a", "#F0A430"))+
...
p4<-dat_sjr|>filter(n_author<20)|>group_by(author_group_five)|> summarise(value= quantile(SJR, c(.7, .8, .90, .99)))|> mutate(percentile = as.factor(c(.7, .8,.9,.99)))|>
  ggplot(aes(x=author_group_five, y = value, color = percentile))+
  geom_point(size= 2)+
  theme_classic(base_size = 11)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))+
...
plots<- plot_grid(p1, p2,p3, p4, nrow = 4)

plot_grid(plots, legend, ncol = 2, rel_widths = c(1, 0.18))
```
...
p1_legend<- dat|>filter(n_author<20)|>group_by(author_group_ten)|> summarise(value= quantile(citation, c(.7, .8, .90, .99)))|> mutate(percentile = as.factor(c(.7, .8,.9,.99)))|>
...
p1<- dat|>filter(n_author<20)|>group_by(author_group_ten)|> summarise(value= quantile(citation, c(.7, .8, .90, .99)))|> mutate(percentile = as.factor(c(.7, .8,.9,.99)))|>
  ggplot(aes(x=author_group_ten, y = value, color = percentile))+
  geom_point(size= 2)+
  theme_classic(base_size = 11)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))+
...
p2<- dat|>filter(n_author<20)|>group_by(author_group_ten)|> summarise(value= quantile(corr_citation, c(.7, .8, .90, .99)))|> mutate(percentile = as.factor(c(.7, .8,.9,.99)))|>
  ggplot(aes(x=author_group_ten, y = value, color = percentile))+
  geom_point(size= 2)+
  theme_classic(base_size = 11)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))+
...
#| fig-cap: "Percentiles of total citation, second-year citation, date-corrected citation, and SJR metrics based on group size of 15"
#| fig-align: center
#| fig-width: 8
#| fig-height: 8
...
p1_legend<- dat|>filter(n_author<20)|>group_by(author_group_fifteen)|> summarise(value= quantile(citation, c(.7, .8, .90, .99)))|> mutate(percentile = as.factor(c(.7, .8,.9,.99)))|>
...
p1<- dat|>filter(n_author<20)|>group_by(author_group_fifteen)|> summarise(value= quantile(citation, c(.7, .8, .90, .99)))|> mutate(percentile = as.factor(c(.7, .8,.9,.99)))|>
  ggplot(aes(x=author_group_fifteen, y = value, color = percentile))+
  geom_point(size= 2)+
  theme_classic(base_size = 11)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))+
...
p2<- dat|>filter(n_author<20)|>group_by(author_group_fifteen)|> summarise(value= quantile(citation_2y, c(.7, .8, .90, .99)))|> mutate(percentile = as.factor(c(.7, .8,.9,.99)))|>
  ggplot(aes(x=author_group_fifteen, y = value, color = percentile))+
  geom_point(size= 2)+
  theme_classic(base_size = 11)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))+
...
p3<- dat|>filter(n_author<20)|>group_by(author_group_fifteen)|> summarise(value= quantile(corr_citation, c(.7, .8, .90, .99)))|> mutate(percentile = as.factor(c(.7, .8,.9,.99)))|>
  ggplot(aes(x=author_group_fifteen, y = value, color = percentile))+
  geom_point(size= 2)+
  theme_classic(base_size = 11)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))+
...
p4<- dat_sjr|>filter(n_author<20)|>group_by(author_group_fifteen)|> summarise(value= quantile(SJR, c(.7, .8, .90, .99)))|> mutate(percentile = as.factor(c(.7, .8,.9,.99)))|>
  ggplot(aes(x=author_group_fifteen, y = value, color = percentile))+
  geom_point(size= 2)+
  theme_classic(base_size = 11)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))+
...
ci_t_15<-wilcox.test(bigauthor_performance_5_15$citation_median_big, bigauthor_performance_5_15$citation_median_small, alternative = "greater", conf.int = TRUE)
p_val<- apa_p(ci_t_15[["p.value"]])

ci_2_15<-wilcox.test(bigauthor_performance_5_15$citation_2y_median_big, bigauthor_performance_5_15$citation_2y_median_small, alternative = "greater",conf.int = TRUE)

ci_c_15<-wilcox.test(bigauthor_performance_5_15$corr_citation_median_big, bigauthor_performance_5_15$corr_citation_median_small, alternative = "greater", conf.int = TRUE)

ci_s_15<-wilcox.test(bigauthor_performance_sjr5_15$SJR_median_big, bigauthor_performance_sjr5_15$SJR_median_small, alternative = "greater", conf.int = TRUE)
```
...
ci_t_25<-wilcox.test(bigauthor_performance_5_25$citation_median_big, bigauthor_performance_5_25$citation_median_small, alternative = "greater", conf.int = TRUE)

ci_2_25<-wilcox.test(bigauthor_performance_5_25$citation_2y_median_big, bigauthor_performance_5_25$citation_2y_median_small, alternative = "greater",conf.int = TRUE)

ci_c_25<-wilcox.test(bigauthor_performance_5_25$corr_citation_median_big, bigauthor_performance_5_25$corr_citation_median_small, alternative = "greater", conf.int = TRUE)

ci_s_25<-wilcox.test(bigauthor_performance_sjr5_25$SJR_median_big, bigauthor_performance_sjr5_25$SJR_median_small, alternative = "greater", conf.int = TRUE)
```

Regardless of the definition for large teams (i.e., number of authors more than 15 and 25), larger teams papers are consistently authors’ more successful publications. The Wilcox rank-sum test was significant for all the tests at the level of `r p_val`. For the first case (i.e., large teams define as having 15 or more authors) we had `r nrow(bigauthor_performance_5_15)` eligible authors for citation metrics and `r nrow(bigauthor_performance_sjr5_15)` authors for journal ranking for the test. The results of the test of medians for small-team publication and large-team publications of authors were W = `r ci_t_15[["statistic"]][["W"]]`, CI = \[`r round(ci_t_15[["conf.int"]][[1]], 3)`, $\infty$) for total citation, W = `r ci_2_15[["statistic"]][["W"]]`, CI = \[`r round(ci_2_15[["conf.int"]][[1]], 3)`, $\infty$) for second-year citation, W = `r ci_c_15[["statistic"]][["W"]]`, CI = \[`r round(ci_c_15[["conf.int"]][[1]], 3)`, $\infty$) for date-corrected citation, and W = `r ci_s_15[["statistic"]][["W"]]`, CI = \[`r round(ci_s_15[["conf.int"]][[1]], 3)`, $\infty$) for SJR. Regarding the second definition of large teams (i.e., large teams define as having 25 or more authors), we had `r nrow(bigauthor_performance_5_25)` and `r nrow(bigauthor_performance_sjr5_25)` eligible authors for citation metrics and journal rankings. Similarly, the results of Wilcox rank-sum test was significant for all the tests at the level of `r p_val`. (median of total citation (W = `r ci_t_25[["statistic"]][["W"]]`, CI = \[`r round(ci_t_25[["conf.int"]][[1]], 3)`, $\infty$)), second-year citation (W = `r ci_2_25[["statistic"]][["W"]]`, CI = \[`r round(ci_2_25[["conf.int"]][[1]], 3)`, $\infty$)), date-corrected citation (W = `r ci_c_25[["statistic"]][["W"]]`, CI = \[`r round(ci_c_25[["conf.int"]][[1]], 3)`, $\infty$)), and SJR (W = `r ci_s_25[["statistic"]][["W"]]`, CI = \[`r round(ci_s_25[["conf.int"]][[1]], 3)`, $\infty$)).

When large teams are defined as having 15 or more authors, the median number of citations for large-team publications in their second year is, on average, `r unstand_ES_15` higher than for publications by smaller teams (five or fewer authors). This number for total citation and publication-date–corrected citation are `r unstand_ES_citation_15` and `r unstand_ES_corr_15` respectively. Similarly, the median SJR of large-team publication of researchers is, on average `r unstand_ES2_15` higher than their smaller group publications. For author group of $\ge 25$ of researchers, these differences are `r unstand_ES` for second-year citation, `r unstand_ES_citation` for total citation, `r unstand_ES_corr` for publication-date–corrected citation, and `r unstand_ES2` for the journal ranking.
