...
dat |>
  ...
  geom_vline(xintercept=c(2014,2024), linetype="dashed", colour = "darkred")+
...
Even though rank correlation among number of authors and SJR value is not high ($\rho$ = `r rho1`), @fig-des-journal shows that mean and median SJR value increase consistently until 56-60 authors bin, then fluctuate for larger groups.
...
ggplot(df_long, aes(x = team, y = value, fill = team)) +
  geom_boxplot(outliers = FALSE) +
  facet_wrap(~ metric, labeller = as_labeller(metric_labs), scales = "free_y") +
  labs(x = NULL, y = NULL) +
  theme_classic() +
  theme(legend.position = "none")+
  scale_fill_manual(values = c("#007E2F", "#BA6E1D"), guide = "none")
