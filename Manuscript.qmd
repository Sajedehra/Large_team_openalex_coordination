...
![Data Cleaning Process](figures/manuscript/flowchart.png){#fig-preprocessing}
...
Our goal was to code 500 valid records, while at the same time using the qualitative checks to identify papers that did not meet the inclusion criteria of this study. We analyzed 647 papers in total over three rounds using MAXQDA 2022 software [version 22.1.0, @verbi_software_maxqda_2021]. In the first round, the first author assessed paper quality and conducted an initial coding of relevant papers based on the reason for large-team collaboration. Only ambiguous cases (n = 8), in which it was unclear whether a paper qualified as psychology research, were discussed with the last author. From the 146 papers that did not meet the inclusion criteria, we distilled keywords that were used in the second round of data cleaning (see @fig-preprocessing).

In the second iteration, the first author reviewed all papers again and refined the initial codes into clearer categories. When the aim of a paper was not clear from the abstract, the methods and results sections were manually examined. The first author then conducted an additional coding round to assign papers to overarching interdependency categories. During this stage, the CREDIT statement (when available) was examined for papers with no clear rationale for large-team collaboration, in order to gain insight into the roles of individual authors. During this process, we identified one paper with incorrect metadata: a publication with eight authors had been coded as having 45 authors. This record was manually removed. However, it is possible that additional metadata errors remained undetected among large-team publications.

In this iteration, the last author conducted two additional coding procedures. First, the last author reviewed entire papers initially coded as having no clear interdependencies (n = 53). Discrepancies emerged for several reasons, including revisions to what counted as a criterion for interdependencies (e.g., the threshold for qualifying as large-scale data collection), initial misunderstandings between two concepts of why authors collaborated and whether a collaboration required many authors, and errors in identifying interdependencies. Based on this review, the “no clear interdependency” category was divided into two subcategories (“No clear reason” and “Part of a larger consortium”), and the remaining papers (n = 33) were reassigned to other categories.

Second, the last author conducted a blind quality-check coding on a randomly selected subset of papers (n = 54). Agreement was reached for 24 papers, while 21 papers showed disagreement regarding the addition or removal of a single interdependency. The remaining nine papers involved disagreements on more than one interdependency. Through discussion, we identified two main sources of disagreement: incomplete reporting of interdependencies in the papers and the difficulty of defining a clear threshold for when a component was sufficiently strong to constitute evidence of an interdependency. To ensure consistency across the dataset, we retained the codes assigned by the first author for papers with discrepancies. Overall, these challenges highlight the need for future research to clarify the boundaries between different types of interdependencies (e.g., financial and logistical dependencies often co-occur, but it is often ambiguous if data collection is costly enough to warrant a large team of co-authors). Furthermore, it is often ambiguous whether a study had sufficient epistemic interdependencies to need a large team of authors to address a research question. These ambiguities deserve attention in future research, and removing them requires that authors disclose additional information about the extent to which interdependendencies were present.
...
eul_graph<- euler(c(
  ...
  "Logistic&Temporal" = 16,
  "Temporal&Epistemic" = 29,
  "Temporal&Epistemic&Logistic" = 10,
  "Financial&Logistic&Epistemic" = 9,
  "Financial&Temporal&Epistemic" = 6,
  "Financial&Logistic&Temporal" = 32,
  "Financial&Logistic&Temporal&Epistemic" = 8,
  "Logistic&Epistemic" = 12
))
...
dat|>filter(n_author<20)|>group_by(author_group_ten)|> summarise(value= quantile(citation_2y, c(.7, .8, .90, .99)))|> mutate(percentile = as.factor(c(.7, .8,.9,.99)))|>
  ...
  ylim(c(0, 150))+
  theme_classic()+
  theme(axis.text.x = element_text(vjust = 0.5, hjust = 1, size= 13))+
...
Codes and materials to recreate this manuscript is available at [https://github.com/Sajedehra/Large_team_openalex_coordination](https://github.com/Sajedehra/Large_team_openalex_coordination). Data used in this study is available at [https://data.4tu.nl/private_datasets/NNOaIjnIG5G1i-kvzILsxBXJD-NV4dqs3UlQw6CfjUc](https://data.4tu.nl/private_datasets/NNOaIjnIG5G1i-kvzILsxBXJD-NV4dqs3UlQw6CfjUc).
