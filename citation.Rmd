...
print(corr)
cor.test(dat$n_author, dat$citation, method = "spearman", use = "complete.obs")
...
```{r}
dat|> group_by(author_group_ten)|> summarise(value= quantile(citation_2y, c(.7, .8, .90, .99)))|> mutate(percentile = as.factor(c(.7, .8,.9,.99)))|>
