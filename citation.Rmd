...
```{r}
duun_five<- dunnTest(citation ~ author_group_five, data = dat_cleaned, method = "bh")
duun_five$res[order(-duun_five$res$Z),]

saveRDS(duun_five, "dunn_five.rds")
saveRDS(duun_fif, "dunn_fif.rds")
saveRDS(duun_ten, "dunn_ten.rds")
```
...
```{r}
duun_ten<- dunnTest(citation ~ author_group_ten, data = dat_cleaned, method = "bh")
...
```{r}
duun_fif<- dunnTest(citation ~ author_group_fifteen, data = dat_cleaned, method = "bh")
...
dat|> filter(n_author>=20)|> summarise(mean = mean(citation_2y), median = median(citation_2y))
