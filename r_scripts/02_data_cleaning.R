...
dat <- fread(here("data","raw_data_OpenAlex.csv"), sep = ";", col.names = c("oa_id", "doi", "pubdate", "title", "journal", 
...
#check1<- sample_n(dat, 200, replace = FALSE)
...
# there are unrealated articles in bmj, but also PRISMA is there (but we decided that PRISMA is not directly related to psychology)
...
dat <- dat |>
  filter(!str_detect(journal, regex(pattern2, ignore_case = TRUE))) # 652812 cases removed
...
saveRDS(dat, file=here("data","first_cleaned_data_openalex.rds"))



# cleanings after qualitative coding ---------------------------------------------------------

#dat<- readRDS("data/cleaned_data_openalex.rds")
