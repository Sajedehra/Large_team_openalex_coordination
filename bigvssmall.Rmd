...
dat_small <- dat_sjr|>filter(n_author<15)
dat_big<- dat_sjr|>filter(n_author>=15)
...
author_performance_SJR2 <- big_authors %>%
...
```{r}
bigauthor_performance_sjr<- author_performance_SJR2|> filter(map_int(small_project, nrow) > 1)
...
saveRDS(bigauthor_performance_sjr5, "bigauthor_performance_sjr5_15.rds")
```
...
df_long|> filter(metric == "citation_2y_median")|>group_by(team)|> summarise(mean = mean(value),sd = sd(value))
