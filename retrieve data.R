...
process_file_in_chunks <- function(file_path, chunk_size = 1000) {
  # Temporary list to store results for each file
  temp_final <- list()
  con <- file(file_path, open = "r")
  on.exit(close(con))  # Ensure the connection is closed when done
  ...
  repeat {
    ...
    for (l in seq_along(lines)) {
      tryCatch({
        ...
        # psychology is either a concept or field name
        is_psychology <- (
  ...
  return(temp_final)  # Return the results of this file
  
}



dat_file <- "D:/Sajedeh/works"
...
for (j in 1:length(dat_file2)) {
  ...
  remove(results_list)
  write_delim(final, file = "dat_field_nn_en2.csv", delim = ";", append = TRUE)
  final <- empt_final
}
toc()
